...
func (t *tgtTarget) AddLun(lun int, path string) error {
	...
	if _, err := t.execCmd(cmd); err != nil {
		log.Println("Fail to exec 'tgtadm' to add lun into iscsi target:", err)
		return err
	}
...
func (t *tgtTarget) GetLun(path string) int {
	...
	if err != nil {
		log.Println("Fail to exec 'tgtadm' to display iscsi target:", err)
		return -1
	}
...
func (t *tgtTarget) RemoveLun(lun int) error {
	...
	if _, err := t.execCmd(cmd); err != nil {
		log.Println("Fail to exec 'tgtadm' to remove lun from iscsi target:", err)
		return err
	}
...
func (t *tgtTarget) CreateISCSITarget() error {
	...
	if _, err := t.execCmd(cmd); err != nil {
		log.Println("Fail to exec 'tgtadm' to create iscsi target:", err)
		return err
	}
...
func (t *tgtTarget) RemoveISCSITarget() error {
	...
	if _, err := t.execCmd(cmd); err != nil {
		log.Println("Fail to exec 'tgtadm' to forcely remove iscsi target:", err)
		return err
	}
...
func (t *tgtTarget) BindInitiator(initiator string) error {
	...
	if _, err := t.execCmd(cmd); err != nil {
		log.Println("Fail to exec 'tgtadm' to bind iscsi target:", err)
		return err
	}
...
func (t *tgtTarget) UnbindInitiator(initiator string) error {
	...
	if _, err := t.execCmd(cmd); err != nil {
		log.Println("Fail to exec 'tgtadm' to unbind iscsi target:", err)
		return err
	}
